...
Заменяет openai_service.py — логика идентична,
только провайдер другой.
"""
...
# in-memory история диалогов: { user_id: [{"role": ..., "content": ...}] }
...
async def get_reply(user_id: int, user_message: str) -> tuple[str, bool]:
    """
    Возвращает (ответ_бота, нужно_ли_передать_сотруднику).
    Флаг True — когда бот явно не нашёл ответа в базе знаний.
    """
    ...
    try:
        response = await _client.messages.create(
            model=CLAUDE_MODEL,
            max_tokens=600,
            system=_system_prompt(),      # system — отдельный параметр, не в messages
            messages=history,
        )
        ...
        # Детектируем «не знаю» → запись в unanswered
        no_answer_markers = [
